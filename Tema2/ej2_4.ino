...
const int distanciaSegura = 10; // cm
      ...
      if (tecla == 'C') {
        ...
        bloqueoEntrada = false;
          ...
          if (distancia < distanciaSegura) {
            // Reinicia el temporizador si hay alguien
            Serial.println("Persona detectada. Manteniendo puerta abierta.");
          tiempoApertura = millis(); // reiniciar tiempo de apertura
          } else {
              cerrarPuerta();
              bloqueoEntrada = false;
          }
...
long medirDistancia() {
  ...
  long distancia = duracion * 0.034 / 2; // Conversión a centímetros
